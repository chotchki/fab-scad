//! fab-types — the shared geometry vocabulary (M.0.2.1): concrete-typed, hand-rolled linear
//! algebra (NOT glam — determinism control), op order read from Manifold's `linalg.h` and proven
//! bit-clean against the C++ oracle through R0–R6 BEFORE being lifted here (lift validated code,
//! don't guess). Componentwise ops via macro (no cross-component order subtlety); the ORDERED ops
//! (dot/cross/matmul) are written by hand. TRIG-FREE by design: rotate builders take precomputed
//! sincos — fab-lang's trig (OpenSCAD `func.cc` reference) and fab-manifold's (`math.h` reference)
//! target DIFFERENT oracles and must never unify.
#![forbid(unsafe_code)]
...
impl Mat3x4 {
    ...
    /// Euler rotation from PRECOMPUTED per-axis `[sin, cos]` pairs, about x, then y, then z —
    /// `CsgNode::Rotate`, `rZ·rY·rX` with zero translation, per-axis matrices column-major exactly
    /// as the C++. Takes sincos rather than degrees ON PURPOSE: this crate is trig-free — each
    /// consumer feeds its own deterministic dialect (fab-manifold shims `mathf::sind/cosd` in via
    /// `linalg::rotate_xyz_degrees`). The COMPOSITION order lives here, once.
    pub fn rotate_from_sincos(sc: [[f64; 2]; 3]) -> Mat3x4 {
        let [[sx, cx], [sy, cy], [sz, cz]] = sc;
...
impl Mat2x3 {
    ...
    /// Z-axis rotation from a PRECOMPUTED `(sin, cos)` pair (`CrossSection::Rotate`'s matrix) —
    /// trig-free like [`Mat3x4::rotate_from_sincos`]; the consumer supplies its dialect.
    #[inline]
    pub fn rotate_from_sincos(s: f64, c: f64) -> Mat2x3 {
        Mat2x3 {
            x: Vec2::new(c, s),
            y: Vec2::new(-s, c),
            w: Vec2::ZERO,
...
impl Vec2 {
    ...
    /// To an `[x, y]` array (the FFI boundary — the 2D subsystem speaks `[f64; 2]`).
    #[must_use]
...
impl core::ops::Index<usize> for Vec2 {
    type Output = f64;
    /// Component access: `0`→x, `1`→y. Panics out of range, like slice indexing.
    #[allow(
        clippy::panic,
        reason = "an out-of-range component index is a bug — panics by contract, exactly as [f64; 2] does"
    )]
    fn index(&self, i: usize) -> &f64 {
        match i {
            0 => &self.x,
            1 => &self.y,
            _ => panic!("Vec2 index {i} out of range (0..2)"),
        }
    }
}
...
mod tests {
    ...
    fn transform_builders_and_composition() {
        ...
        let rz90 = Mat3x4::rotate_from_sincos([[0., 1.], [0., 1.], [1., 0.]]);
        ...
        let a = Mat3x4::rotate_from_sincos([[0., 1.], [0., 1.], [1., 0.]]).linear();
        let b = Mat3x4::rotate_from_sincos([[1., 0.], [0., 1.], [0., 1.]]).linear();
