...
pub fn generate_standalone_modules(root: &std::path::Path) -> Result<String, String> {
    ...
    let band: Vec<_> = lib
        .modules
        .values()
        .filter(|m| generate_module_native(&m.source, &[], &[]).is_ok())
        .collect();
    let refs: Vec<&str> = band.iter().map(|m| m.source.as_str()).collect();
    let mut out = generate_modules(&refs)?;
    out = out
        .replace(
            "GENERATED by `fab_lib::emit::generate_modules` (AR.20.8)",
            "GENERATED by `fab_lib::emit::generate_standalone_modules` (AR.14.4 band 1)",
        )
        .replace(
    ...
    let _ = write!(
        ...
        "\n/// The band's registry — chained into `module_table` after the POC entries. Verbatim\n\
         ...
         /// from the pinned library interprets instead of wiring.\n\
