...
- [ ] AA.4 - Parser depth (issue4172, chotchki's call 2026-07-22: the ITERATIVE SPINE, not a budget bump — the M-phase heap-bounded doctrine, parser edition): the 144-deep vector literal (and arbitrarily deeper) parses without stack death, the cliff class dies rather than moves
  ...
  - [ ] AA.4.2 - The explicit-stack expression parser: heap-allocated work stack replaces call recursion for the deep dimensions; MAX_DEPTH stays for STATEMENTS (module/if nesting — a different, legitimately-bounded shape) or is re-scoped deliberately
  - [ ] AA.4.3 - Downstream consumers survive the new depth: fingerprint/print/Drop audited + fixed or explicitly budgeted; eval already heap-bounded (M) — verify with the real issue4172 eval shape (their guard errors gracefully; ours must too, not abort)
  - [ ] AA.4.4 - Gates: issue4172 parses + evals to a clean error; a fuzz seed at ≥10× the old cliff; parser fuzz target exercises deep nesting; census re-sweep
