...
- [x] AN.10 - intrinsics: is_vector's `all_nonzero` PARAMETER shadows the like-named FUNCTION in call position; the intrinsic hardcodes the function
- [x] AN.11 - intrinsics: const guard now also checks each DEP's consts against the DEP's own island (one hop) — sound by construction, but NOT demonstrated by a failing repro; see AN.17
...
- [x] AN.14 - diagnostics, call sites: `argument "a" supplied more than once` + `variable "x" not specified as parameter` — values already agree; needs ctx threaded into push_call (2 callers) via a warn-only helper, NOT fill_arg_slots (the intrinsic gate calls it too and would double-warn)
- [ ] AN.15 - diagnostics needing SPAN plumbing: `"a" was assigned on line N but was overwritten` + `Parameter "x" is overwritten with a literal`. GROUNDWORK DONE: Stmt.span (byte span) and fab_lang::offset_to_line both exist; the blocker is that eval's Ctx has NO source field, so the line can't be resolved where the warning is emitted — thread a line table into Ctx, or emit span-carrying messages and resolve at the driver seam like W.3.37 does
  - [ ] AN.15.1 - STATIC dup-assignment warning `"a" was assigned on line N but was overwritten` — oracle-pinned: fires for UNCALLED module bodies (p1) and BEFORE all eval output (r1), so it is a static AST pass, not an eval-time warning; cited line is the FIRST assignment, trailer moves (q3); bare `{}` flattens into the parent scope (s3), `if` branches are their own scope (q4/s1). Root file only — the ` of "path"` multi-file variant is AN.15.3
  - [ ] AN.15.2 - `Parameter "x" is overwritten with a literal` — needs NO span at all (no line in the body); fires at INSTANTIATION, once per call (r2), module params only not function (r3), only the body's flattened top-level scope not nested `if` (r4), defaultless params too (r5), in BODY-ASSIGNMENT order not declaration order (o1). "Literal" = upstream Expression::isLiteral: literal, unary-op forwarding to its operand, vector/range iff all elements literal; binary op is NOT (q1/r6)
  - [ ] AN.15.3 - Multi-file variant of the dup-assignment warning: a non-root file renders `"d" was assigned on line 1 of "lib/dup.scad" but was overwritten` — the path sits INSIDE the body so differ::normalize_warning can't strip it, and `use`+`include` of the same file warns TWICE. Open question: what is the path relative to (as-written vs resolved)? Needs its own oracle probe before implementing
- [x] AN.16 - fuzz: a DISPATCH-level differential target (jit=off vs jit=on over generated programs) — jit_diff binds params positionally and calls compile_function directly, so it structurally cannot see AN.2/AN.3/AN.4/AN.5; all four needed hand-finding
- [ ] AN.17 - AN.11 follow-up: build a REPRO for the cross-island dep-constant case — its known one went through is_vector, which AN.10 now vetoes outright; posmod/approx diverges only via an assert, which diff_echo can't see (both sides go empty). Needs a value-carrying dep, or an error-comparing channel
- [ ] AN.18 - differ: a fab-lang EVAL ERROR still reads as an empty console — `FabLang::messages` ends in `unwrap_or_default()`, so a program that fails to evaluate is indistinguishable from one that printed nothing, and any `agree_echo`/`agree_warnings` case where the oracle is also quiet passes VACUOUSLY. Found while fixing AN.15.1 (the mesh_of half is fixed; this half is not). Needs a decision on the error contract: compare an error MARKER on both legs (the oracle errors too on genuinely-bad input), vs fail loud — blast radius unknown, existing cases like a_missing_use_warns_and_renders_like_the_oracle deliberately exercise error paths
