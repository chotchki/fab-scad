...
## Phase AJ - Generator coverage: the grammar emits the WHOLE language (chotchki 2026-07-23: "concerned it's still thin" — it is)
  Meta - fab-gen (K.3) is v0-thin: assignments, arity-N function defs, 3 primitives, 3 booleans, 8 expr forms. NO modules (no defs/children()/$children), no let/assert/echo chains, no each/if/C-style comprehensions, no indexing/member/swizzles, no function LITERALS (closures/letrec/methods), no objects, no $-vars/$-args, no named args, no modifiers, no if/else statements, no 2D/extrudes, no bit ops. And it feeds ONLY the R.1 perf manifest — the fuzz lane never sees it. Three legs: a MEASURABLE coverage gate (test-first — its failing list is the inventory), the grammar itself, and fuzz-lane wiring with the Config contract (caches/JIT A/B bit-identity) as the oracle for generated programs. Rails throughout: everything bounded + hermetic (no fs — file-fns emitted against NONEXISTENT paths exercise the warn+undef channel only), seeded rands only, $fn capped, no self-recursive generated bodies.
- [x] AJ.1 - the coverage GATE: a fab-gen test asserting every construct-family MARKER appears across N seeds (module defs, children(), each, C-for, .swizzle, object(, function(, let(, assert(, echo(, $fn=, named args, %/#/! modifiers, linear_extrude, textmetrics, seeded rands, …) — written against the FULL surface so it fails NOW and drives AJ.2-AJ.5; future language additions that skip the generator fail CI here
- [x] AJ.2 - statements: user MODULE defs + calls (params/defaults/children()/children(i)/$children/nested defs), if/else, for + intersection_for statements, let/assert/echo statements, %#!* modifiers, $-assignments + $-args at call sites
- [x] AJ.3 - expressions: let/assert/echo CHAINS (+ duplicate let names — the AH.2.3 rule), each/if/C-style comprehensions + nesting, indexing + member access + multi-letter swizzles, function LITERALS (closures over locals, letrec by named assignment, immediate calls), named args + positional-after-named + unknown named, bit ops/shifts/^/%, undef/inf/nan atoms, unicode + escape strings
- [x] AJ.4 - values/builtins: object() all forms (named/copy/edit-lists incl. removes), method calls (this), has_key/is_object/len/key-iteration, textmetrics/fontmetrics (bundled font — deterministic), str/chr/ord/search/lookup/concat/reverse, seeded rands, is_* family, expression import + dxf_dim on nonexistent paths (hermetic warn+undef)
- [x] AJ.5 - geometry: 2D primitives (square/circle/polygon/text) + linear_extrude/rotate_extrude/offset/projection/color/resize/scale/mirror/multmatrix/hull/minkowski — small bounded args, $fn capped
- [ ] AJ.8 - ORACLE differential + TIMING (chotchki 2026-07-23): run each generated program through the LOCAL OpenSCAD binary AND ours — diff the echo output (the AH-lane comparison logic) and report wall-time ratio with startup-baseline subtraction; classify known-divergence constructs (textmetrics needs the oracle's --enable flags + font env) — this is R.2's "values/echo first" made real
- [ ] AJ.6 - fuzz-lane wiring: new structure-aware `gen_diff` target (bytes → seed → generate → eval with Config::default() vs caches-on, assert bit-identical messages + geometry fingerprint — the Config contract AS the oracle) + regenerate the parse/eval seed corpora from the fattened grammar; fuzz.yml carries the new target
- [ ] AJ.7 - soak + triage: 100% parse-validity over a large seed sweep, N-seed A/B soak, timed runs on each fuzz target; findings fixed or filed; distribution weights documented so rare arms provably fire
