...
# Next Major Spec
- GUI changes (these steps should still be CLI verbs too, the gui is just making setting these values easier)
  - Meta:
    - gui remembers last folder it was used against
    - more guided flow is the goal
    - The web AND the local desktop version should as similar as possible
    - I'm unsure if navigation should be top tabs or a left accordian (leaning towards top tabs + left panel)
      - In auto mode, the tabs should show the working animations as the dag gets updated
    - If auto process is checked, jump to to the plates tab after a file is selected, the user can always go back and adjust anything they want
  - Tabs (In order)
    - Config - (Can be auto set via url param on web)
      - checkbox with auto process checked
      - printer config 
    - Loading - (Can be auto set via url param on web)
      - File / Folder Picker (In theory folder picker should work in the web but that could be a backlog item)
      - This is where you can open openscad
    - Planes - (Can be auto set via url param on web)
      - The X / Y / Z Plane picker / adjuster
      - Set Connectors
        - still per plane button
        - shows the 2d outline of the cross section
        - same connector type pickers / autoplace like today
    - Plates - (Can be auto set via url param on web ... this may be hard)
      - Sliced / Rotated
      - allows the manual override
