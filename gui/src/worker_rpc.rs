...
//! onerror that fails every pending call. Ported verbatim from fab-web, where the review refuted the
//! one-shot transport twice over: a reply arriving after its call was dropped consumed ANOTHER call's
//! once-closure (crossed replies, wrong model under the picked name), and a 404'd worker script never
//! resolved anything (eternal busy pulse, every button "still working"). Now a stale reply resolves a
//! dead promise harmlessly, and a load failure errors every caller.
