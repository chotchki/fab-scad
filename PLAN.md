...
- [x] AD.2 - Zero-progress recursion → "Recursion detected": SHIPPED AS AN IN-FLIGHT CALL COUNTER, not arg-cycle detection — recursion-test-function3 (`add_up_to(-1)`) recurses with CHANGING args and upstream still errors, proving their guard is a counter (the TCO loop caps at 1,000,000); ours counts interpreted calls in flight (`live_calls` + a LIFO `CallReturn` task), ceiling 1M = upstream's, 2× the 500k legit chain; all 4 census files → fast named "Recursion detected" (was 4×~123s timeouts)
- [x] AD.3 - Range too-many-elements: `range_len >= u32::MAX` ⇒ warn + ZERO iterations (boundary pinned by for-tests' own annotations — count EXACTLY u32::MAX already warns); for/each/comprehension seams only, builtins (chr/lookup/search) + intrinsics keep the raw capped expansion (`iter_values_raw`); for-tests 26s + errors-warnings 53s → both eval-clean in 0.66s
- [x] AD.4 - C-style-for cap: hard error PAST 1,000,000 iterations (both edges oracle-probed: exactly 1e6 clean, 1e6+1 "ERROR: For loop counter exceeded limit" — verdict verbatim) replacing the silent RANGE_MAX break; for-c-style-infinite-loop 4.6s grind → verdict-parity error in ~0.4s
- [x] AD.5 - Guard economics: the trade-off DISSOLVED — the ceiling stays 100k (the deep-recursion doctrine untouched) because the real pathologies were elsewhere. (1) Both cache gates (eval + CSG) weighed args SHALLOW, so `rec([a,10,10])` weighed 3 at every depth and ModKey hashed the whole growing vector per call — the O(n²) that made 100k unreachable; now `bounded_weight` counts NESTED size with a cap-bounded walk. (2) issue4172's echo loop needed upstream's own seam: echo REFUSES a >10k-nested value (upstream stack-exhausts converting to EchoString; ours is a deterministic bound). Module guard verdict upgraded Unimplemented→Eval "Recursion detected calling module 'rec'" (upstream's class). Both files: >120s timeouts → 1.16s combined errors at the FULL ceiling; shoe A/B shows no gate regression (2.5s both sides)
- [x] AD.6 - Re-baseline: census re-sweep — 10 of 10 timeouts resolved (3 → eval-clean: for-tests, errors-warnings-included, function-literal-tests; 7 → fast upstream-parity errors), census now 548/576 clean with ZERO timeouts, full 576-file sweep in 2.0s (the timeout bucket alone used to burn 8+ min); sustain nightly picks the numbers up on its next run; release call: v1.0.3 recommended (AD.1 is a real user-facing correctness fix — local closures now shadow named functions in call position), awaiting chotchki's go
