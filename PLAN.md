...
- [x] AJ.8 - `fab gen-diff` SHIPPED: per-seed oracle differential (echo line-diff + startup-adjusted wall time; oracle runs --enable=textmetrics,object-function with a capability probe; version-skew detection for pre-July oracles lacking multi-swizzles). FIRST RESULTS (local 2026.06.12 oracle, 200 seeds): 199/200 echo-match, 1 divergence = VERSION SKEW (multi-letter swizzles absent in that build — probed, not our bug), 0 ours-failed; timing median ours 2.0ms vs oracle 4.9ms adjusted → ~2.5x FASTER on generated micro-programs (fixed overheads dominate; honest claim: never worse, typically faster). Found + fixed a generator 2D/3D top-level mixing bug the oracle rejected (we tolerate mixed top-level — noted laxness)
