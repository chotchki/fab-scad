...
pub fn generate_batch(subjects: &[Subject<'_>], declared: &[&str]) -> Result<String, String> {
    ...
    let mut header = String::from(
        "// GENERATED by `eval::transpile::generate_module` (AR.6) — DO NOT EDIT.\n\
         // Refresh: FAB_REGEN=1 cargo nextest run -p fab-lib -E 'test(generated_file_is_current)'\n\
         //\n\
...
pub const GENERATED_ENTRIES: &[&str] = &[
    ...
    "is_vector",
    "all_nonzero",
    // TC.4 — is_matrix RETIRED here: v2.0.751 rewrote it (tensor rank/shape) and the transpiled
    // band owns it now. Its old sibling callers emit `fx.call_named` (AR.27) from this absence.
    "sum",
    "unit",
    "_apply",
    "_bt_search",
