...
- [ ] AA.4 - Parser depth (issue4172, chotchki's call 2026-07-22: the ITERATIVE SPINE, not a budget bump — the M-phase heap-bounded doctrine, parser edition): the 144-deep vector literal (and arbitrarily deeper) parses without stack death, the cliff class dies rather than moves
  - [ ] AA.4.1 - Recon + design: map every recursion in the expr parse path (precedence cascade, vector/paren/index nesting, unary chains) + the DOWNSTREAM cliffs a deep AST then reaches (fingerprint's hash_expr, print/str formatting, Expr Drop) — the spine is only as good as its weakest consumer; design doc'd before code
