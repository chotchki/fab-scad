...
- **object() — the last genuine census gap (AE.4, 2026-07-23), upstream-EXPERIMENTAL (`--enable object-function`, echo lane only).** Scope from object-tests.scad + its golden: a first-class insertion-ordered string→value map (100k-key perf expectation, so a real map not an assoc list); `object()` ctor forms (named args / copy / list-of-pairs / copy-with-edits `[["k"]]`=remove `[["k",v]]`=set); dot-access POSTFIX SYNTAX `o.a` (incl. `.$fs`); `o["key"]`; key iteration; len/has_key; structural equality; `{ a = 1; }` echo format. The HAIRY half is the `this` mechanic — a function member with a trailing `this` param becomes a METHOD (receiver bound at member-extraction, survives storage into other objects, per-object `this` under recursion, global `this` must NOT leak) — that's method dispatch in the call machinery, not just a Value variant. Variant cost is invasive: equality/echo/str, eval+CSG cache weighing (`bounded_weight` arm), JIT ABI exclusions, fuzzer grammar. DEFERRED on purpose: nothing in BOSL2/models/the workflow touches it, and the surface can churn under the experimental flag — the sustain watch flags golden churn, implement when upstream de-experiments (or a corpus/BOSL2 consumer appears). If implemented: acceptance = the golden echo diff, NOT eval-clean (the test's PASS/FAIL echoes don't assert — a partial impl would "pass" the census bar while printing FAIL).
