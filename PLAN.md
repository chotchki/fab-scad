...
- [ ] W.3 - One codebase: fab-gui on desktop AND web at FULL PARITY (web JUST AS FUNCTIONAL as desktop), retire fab-web. The kernel becomes a byte GEOMETRY-SERVICE behind one seam — a thread on native, a Web Worker on wasm (fab-web's already-proven `geomsvc` model). Sequence: SERVICE-first → DESKTOP-first (the fast test/dogfood loop) → wasm as a transport swap (the slow browser/toolchain loop LAST). scad-rs+Manifold everywhere, NO OpenSCAD-wasm.
  ...
  - [ ] W.3.3 - NATIVE transport [design LOCKED: docs/geometry-service.md — 5-op seam (only Solid-touching ops cross; feasibility/copack/export stay in-process), STATEFUL shard-tagged SolidId handles (RETAINED not consumed), enum pool transport (N=1 now, shards designed in), respawn-on-crash] + fab-gui refactor onto the seam: a persistent kernel thread + channel; fab.rs/jobs.rs call the SERVICE, not `fab_scad::kernel`; the internal render→slice→orient→pack DAG becomes in-memory bytes (disk drops out except at the true edges). Verify on the FAST loop (588 + dogfood); confirm the desktop WINS too — crash-isolation (a kernel OOM kills the worker, not the app) + no serialize→disk→deserialize between stages. This is where the platform gap closes. **DONE SO FAR (committed, 589 green): (a) SERVER side — geomsg protocol + shard-tagged SolidId + geomsvc SolidStore/handle_with_store + the 6 arms ported [006b17b1]; (b) native TRANSPORT — gui/geom.rs GeomPool kernel-thread pool, tested [e6bea696]. REMAINING = the GUI REFACTOR (one atomic slice): gut fab.rs's kernel fns (keep the pure ConnKind/Conn/Orient/PiecePrint types + in-process copack/export/feasibility); state.rs Part.base_stl:PathBuf→base:SolidId; make GeomPool a cloneable Bevy Resource in lib.rs; jobs.rs/print.rs task bodies → pool.call().await, build meshes from Response STL bytes, issue Free on reset/part-count/refresh_part; then 589 + dogfood.**
  - [ ] W.3.4 - wasm host surface: `#[wasm_bindgen(start)]` + panic hook, `#fab-web` canvas bind + fit-to-parent, the data-* config channel + query params + localStorage, rfd `.read()`-bytes open + Blob download (lift from fab-web); Platform-gate the desktop-only edges (Save/watch/current_exe).
  - [ ] W.3.5 - egui-on-wasm SMOKE (the #1 unknown — now it just falls out): UI + empty scene render in a real browser; verify bevy_egui 0.41 panels/input/clipboard/file surfaces on a real frame. fab-web can't vouch (it shipped feathers).
  - [ ] W.3.6 - WORKER transport: wire the geometry service to a Web Worker (geom-worker.js + worker_rpc from fab-web); the geom worker crate builds manifold-wasm (LLVM-21 + wasm-cxx-shim + binaryen-v130 pins). SAME app code as native — a transport swap. Full geometry on the web.
  ...
  - [ ] W.3.8 - PARITY: config persistence without a filesystem. Design + implement a `fab:config` COMMENT-BLOCK metadata format the app reads/writes in the .scad buffer, so web bakes settings into the downloaded .scad. UNIFY both platforms onto it (chotchki's call: unify web+desktop) — desktop reads/writes the same `fab:config` block and the project.toml/toml_edit autosave path retires. ONE config mechanism, both targets.
  - [ ] W.3.9 - PARITY: parallel Manifold on wasm (the TBB problem — chotchki wants it SOLVED). Build oneTBB for wasm (wasm threads via SharedArrayBuffer; the site's COOP/COEP already satisfy cross-origin isolation) for perf parity, and resolve determinism — TBB parallel_reduce is order-nondeterministic, so either fix Manifold's reductions to a fixed order or document a web divergence (see [[onetbb-wasm-determinism]]). Hard + semi-independent; may PROMOTE to its own phase.
