...
- [ ] AH.2 - divergence burn-down: first census = 60/94 matched, 34 diverged — 5 feature-pending, 1 harness-mode, 1 PRNG, the rest REAL parity bugs in ~10 root-cause clusters eval-clean never saw:
  ...
  - [ ] AH.2.5 - scope cluster: scope-assignment-tests ("outer e (3)" → 3 not 2), variable-scope-tests ($children=1 not 4), children-tests (range order), parent_module-tests (echo order/content)
  - [ ] AH.2.6 - for/range edges: for-tests (ours echoes "never shown" first — ordering or empty-range iteration), range-tests line 65 (we emit an extra line), special-consts trailing undef, examples/Functions/echo.scad (golden leads with bare `ECHO:`)
  - [ ] AH.2.7 - function-literal semantics: literal equality (compare says true upstream), chaining call → 0 not undef
  - [ ] AH.2.8 - errors-warnings ×2: a value after an erroneous assignment reads undef upstream, we compute 10
