...
- [x] AC.2 - 2D minkowski DONE: `CrossSection::minkowski_sum` — the 3D port's tiered hull+union (PR #666) ONE DIMENSION DOWN (tier 0 convex×convex vertex-sum hull; tier 1 sweep-along-EDGES, the 2D boundary element; tier 2 triangulate-both via the shared robust triangulator — the triangulation IS the convex decomposition), original-A core seed, empty identities, sequential-deterministic. Plumbed whole-stack: `Shape2D::Minkowski` + geo_stack/geo_drop/extent-walker arms, backend `minkowski_2d` (annihilating N-ary fold mirroring 3D) + Mock/Counting impls + `Section` wrapper + both geo_hash arms; Combinator::apply is INFALLIBLE now (the last LOUD-defer left the club — two stale tests flipped to tombstones). ORACLE-EXACT: square⊕circle vol 35.121==35.121, concave-L⊕unit-square == 21 closed-form; tier-2 pinned by inclusion-exclusion (52.0 exact); **all 5 census files green**; 807/807 across three crates
- [x] AC.3 - Re-baseline: census **529→545/576** this phase (fonts +13 incl. a runner-borderline, minkowski +4, recursion-probe reshuffle). Named owners: 10 assertion→semantics worklist; 1 unimplemented→the module-recursion-guard probe (guard parity, upstream errors too); 2 parse→verdict parity; 8 load→MCAD + kin (ship-or-skip = a future chotchki call); 10 timeout→recursion-limit probes + issue4172 economics. The sustain nightly carries the baseline on its next run
