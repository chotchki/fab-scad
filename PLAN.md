...
- [ ] L.2 - Burn-down: fixes land as semantics/ tests; expect this to expose evaluator gaps — that's the point
  ...
  - [ ] L.2.8i - L.2.8i - fnliterals f_acos: acos/asin degree divergence — RE-DIAGNOSED 2026-07-07 (was mis-filed as "1-ULP libm, needs deterministic libm crate"). It's a CORRECTLY-ROUNDED-transcendental gap, and a plain-formula tweak does NOT fix it: (a) macOS libm's acos(-0.5) is 2 ULP off the correctly-rounded 2π/3, and the musl `libm` crate is off DIFFERENTLY (they disagree even on acos(0.5)) — verified neither lands on exactly 120. (b) Tried swapping `.to_degrees()` (`r*(180/π)`) for `(r/π)*180` (a candidate rad2deg): FALSIFIED — it made acos(0.5)→60 exactly but REGRESSED test_glued_circles (an acos/atan2 value crossed a `segs()` threshold → structurally different arc discretization, 877→875). So `(r/π)*180` is NOT what the oracle does; the oracle is glibc's correctly-rounded acos + `to_degrees`-equivalent. The real fix is a correctly-rounded inverse-trig impl (glibc-quality / CORE-MATH-class) — deterministic AND oracle-matching — or accept as a documented platform divergence. Manifold's sind/cosd don't help (forward-only — we already do exact quadrants — and platform-libm non-deterministic). Ties to determinism doctrine #36.
  ...
  - [ ] L.2.8m - L.2.8m - module-body-LOCAL function/module definitions (nested-def hoisting + scoping): `make_path`/`qrok`/`nullcheck`/`valid_lock`/`check_path_apply`/`testvercmp` are defined INSIDE a module/test body and read as unknown — the highest-leverage open gap (~6 tests: rounding_edge_mask, fillet, qr_factor, null_space, rabbit_clip, apply, version_cmp)
