...
mod tests {
    ...
    /// Skips when the submodule is absent (the checked-in files still compile everywhere); the
    /// band membership recomputes on every regen, so a module that gains a constant read falls
    /// OUT rather than arming unguarded. One file per BOSL2 source since AR.23: the regen sweeps
    /// files the generator stopped writing, and currency compares the file SET before the bytes —
    /// a stale straggler would compile (its `mod` line is gone) but still reads as armed code.
    /// Refresh with `FAB_REGEN=1`.
    #[test]
    fn generated_bosl2_modules_are_current() {
        ...
        let want: BTreeMap<String, String> = super::generate_standalone_modules(&root)
            .expect("generates")
            .into_iter()
            .collect();
        let dir = std::path::Path::new(concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../lang/src/eval/intrinsics/generated_bosl2_modules"
        ));
        if std::env::var_os("FAB_REGEN").is_some() {
            std::fs::create_dir_all(dir).expect("create generated_bosl2_modules/");
            for entry in std::fs::read_dir(dir).expect("read generated_bosl2_modules/") {
                let path = entry.expect("dir entry").path();
                let stale = path.extension().is_some_and(|x| x == "rs")
                    && path
                        .file_name()
                        .is_some_and(|n| !want.contains_key(&*n.to_string_lossy()));
                if stale {
                    std::fs::remove_file(&path).expect("sweep a file the band no longer writes");
                }
            }
            ...
            if path.extension().is_some_and(|x| x == "rs") {
                have.insert(
                    path.file_name()
                        ...
                        .to_string_lossy()
                        .into_owned(),
                    std::fs::read_to_string(&path).expect("read generated file"),
                );
            }
        }
        assert_eq!(
            ...
            "generated_bosl2_modules/ file set is stale — refresh with FAB_REGEN=1"
        );
        for (name, w) in &want {
            assert_eq!(
                &have[name], w,
                "generated_bosl2_modules/{name} is stale — refresh with FAB_REGEN=1"
            );
        }
    }
