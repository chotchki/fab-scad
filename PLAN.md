...
- [ ] AH.2 - divergence burn-down: first census = 60/94 matched, 34 diverged — 5 feature-pending, 1 harness-mode, 1 PRNG, the rest REAL parity bugs in ~10 root-cause clusters eval-clean never saw:
  ...
  - [ ] AH.2.2 - echo renders strings RAW (no escaping): golden prints `("a")` where we print `(\"a\")` — search-tests, string-test, unicode-tests; check str()/list-nesting blast radius against existing pins
  ...
  - [ ] AH.2.9 - tan(-180) is NEGATIVE zero (exact-angle trig snap misses the sign) — trig-tests line 44
