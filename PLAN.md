...
- [x] AI.1 - value-need plumbing: SourceNeed::Data (path pre-resolved by the eval — a pure lexical join) + Imported::Bytes; fulfill reads bytes ITSELF (io.rs IS the impure shell — zero reader-signature churn, the corpus sweep needed no changes); expression `import()`/dxf_dim/dxf_cross dispatch as Task::FileFn carrying the calling file's dir from the hidden per-island `__fab_file_dir__` binding (bound into island globals AND their pre-hoist placeholders — the L.2.8a ordering gap would otherwise blank it for root constants calling use'd functions, which is exactly the relative-path test's shape)
- [x] AI.2 - JSON → Value via serde_json DEFAULT features (BTreeMap map = upstream's sorted nlohmann std::map, goldens confirm); null → undef, arrays → lists, objects → Value::Object in sorted order; parse errors warn + undef; serde_json lives in fab-lang (pure parsing, wasm-safe) with the dep rationale in Cargo.toml
- [x] AI.3 - dxf_dim + dxf_cross: focused DIMENSION/LINE group-code parser (origin/scale transforms per the reference incl. the 11/12/16 scale-only quirk; BLOCKS/INSERT out of scope) + dxfdim.cc's type&7 case split on our exact-quadrant trig; unit-tested on a synthetic 3-4-5 fixture
- [x] AI.4 - sweep needed NOTHING (Data needs bypass mesh_reader entirely); sustain.yml sparse-checkout += tests/data/json (data.json lives outside tests/data/scad). Web/wasm Data wiring = documented follow-up (map-backed fulfill warns + empty bytes)
- [x] AI.5 - acceptance: import-json, import-json-relative-path (per-containing-file resolution PROVEN — the recorded per-run base-dir divergence is fixed for the function form), and dim-all all match VERBATIM. Census: 551/576 eval clean, echo goldens 94/94, genuine failures = included.scad (an include-fragment upstream never runs) and nothing else
